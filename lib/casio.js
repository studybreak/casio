...
Casio.model = function(name, opts){
    ...
    _.defaults(opts, {
        ...
        // todo: partion this different for reads vs writes 
    ...
    **/
    ...
    /**
        Model.primary - Return the primary key column;

    **/
    Model.primary = function(){
        
        if (this.prototype._primary) {
        ...
        for (var p in this.prototype._schema) {
            ...
            if (prop.primary) break;
        }
        // set this for later;
        this.prototype._primary = p;
        return p
    }

    /**
        Model.property - define a prop with name as type with these options
    ...
    **/
    ...
    /**
        Model.classMethods - Extend class methods
    ...
    **/
    ...
    /**
        Model.instanceMethods - Extend methods as instance methods
            @methods - object of methods
    **/
    ...
    /**
        Model.find - return a list of models
            @args {
    ...
    **/
    Model.find = function(args, callback){
        ...
        var s = q.statement();
        this.cql(s, [], function(err, results){
    ...
    /**
        Model.get 
            ...
            By default, the model is instantiated...
                as the model type this was called from
        
            @args = key
            -or-
            @args = {
                columns:['*'] // defaults to all. 
                                 optional csv or list of specific column names to return
            }
    ...
    **/
        ...
        if (typeof(args) !== 'object'){
            ...
            if (args.where === undefined){
                if (args[primary] === undefined) {
                    return callback(new Error('Missing key in get args object'));
                }
    ...
    /**
        Model.createIndicies - create indicies on all defined properties 
            @callback
    **/
    ...
    /**
        Model.incr - increment a key.column counter this many times
    ...
    **/
    ...
    /**
        Model.decr - decremente a key.column counter this many times
            This is just a pass wrapper for Model.incr and negates the i value
            before calling it...
    ...
    **/
    ...
    /**
        Model.prototype.shadow - take all the properties and sync model._props values
    ...
    **/
    Model.prototype.update = function(callback){
        ...
        // update _props with new changes...
    };
    ...
    **/
    Model.prototype.create = function(callback){
        ...
        if (this.key === undefined){
            this.key = new Cassandra.UUID().toString();
        }
        ...
        for(var p in this._schema){
            ...
            // set created_at
            if(!hasCreatedAt && p.replace(/_/g,'').toLowerCase() === 'createdat'){
                hasCreatedAt=true;
                var dt = new Date();
                this[p] = dt;
                val = dt.getTime();
            ...
            // set updated_at
            else if(!hasUpdatedAt && p.replace(/_/g,'').toLowerCase() === 'updatedat'){
                hasUpdatedAt=true;
                var dt = new Date();
                this[p] = dt;
                val = dt.getTime();
            }
            // make sure we convert all Date values to a timestamp
            else if (colDef.type === Date){
                // 1. check to make sure this[p] is a Date
            ...
            // todo: need to determine how to handle null values
            args[p] = val;                
        ...
        this._client.execute(statement, [], function(err, results){
            ...
            // copy all saved props into _props
            for (var p in self._schema){
                self._props[p] = self[p]
            }
...
CQL.prototype.delete = function(cf){
    this._cmd = 'DELETE';
    this._cf = cf;    
}
