...
var Casio = module.exports.Casio = function (options) {
    ...
    this.connection.on('log', function() {
        self.emit.apply(self, ['log'].concat(Array.prototype.slice.call(arguments)));
    });
...
**/
AbstractModel.outputTiming = function (qry, results, metadata) {
    if (!metadata) return;
    var hash = '"' + qry.slice(0,32) + '"';
    console.log('CassandraTiming', hash,
        metadata.connectionInfo.host,
        metadata.timingInfo.queryLatency,
        metadata.timingInfo.poolLatency,
        qry.length,
        results && results.rowCount && results.rowCount() || 0,
        _.reduce(results, function (m, r) { return m + r._colCount; }, 0));
};
...
AbstractModel.cql = function(qry, args, callback) {
    ...
    this.prototype.connection.execute(qry, args, function (err, results, metadata) {
      AbstractModel.outputTiming(qry, results, metadata);
...
AbstractModel.prototype.cql = function(qry, args, callback) {
    ...
    this.connection.execute(qry, args, function (err, results, metadata) {
      AbstractModel.outputTiming(qry, results, metadata);
      (callback || nullCallback)(err, results);
    });
};
    ...
    **/
    ...
    ModelArray.prototype.multiQuery = function (args, callback) {
      ...
      async.map(this._options.shards, function (shard, callback) {
        ...
        if (err) return callback(err);
        callback(null, mergeRows(results));
      });
    };
...
var mergeRows = function (rows) {
  ...
  while (true) {
    for (i = 0, minIndex = null, minItem = null; i < rows.length; i++) {
      ...
      if (!minItem || item.name < minItem.name) {
        minItem = item;
        minIndex = i;
      }
    }
