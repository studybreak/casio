...
var Casio = module.exports.Casio = function (options) {
    ...
    this.connection.on('log', function() {
        self.emit.apply(self, ['log'].concat(Array.prototype.slice(arguments)));
    });
...
Casio.prototype.model = function(name, opts) {
    ...
    **/
    Model.find = function (args, callback) {
        ...
        // keyAlias being diff from the primary...
        if (args.where && 
            args.where instanceof Array && 
            Model.keyAlias !== primary){
              var re = new RegExp(primary + '(?!\\w)', 'g');
              args.where[0] = args.where[0].replace(re, Model.keyAlias);
        }
        ...
        if (args.where !== undefined){
            if (args.where instanceof Array) {
                ...
                var bind = (args.where.length > 1) ? args.where[1] : undefined;
                q.where(clause, bind);
