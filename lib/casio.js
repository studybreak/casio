    ...
    **/
    Model.connect = function(callback){
        ...
        this.prototype._client.on('log', function(level, message, obj) {
          console.log('CLIENT[%s] %s -- %j', self.prototype._name, level, message);
        });        
...
Casio.array = function(name, opts){
    ...
    **/
    ...
    /**
        ModelArray.classMethods

            Extend class methods

        @methods    - object of methods
    **/
    ModelArray.classMethods = function(methods){
        // _.extend(this, methods)
        for (var name in methods){
            if (this[name] !== undefined){
                throw new Error('Falied to extend ModelArray.classMethod. ' +
                    this.prototype._name + '#' + name + '() already exists.');
            }
            this[name] = methods[name];
        }
    ...
    /**
        ModelArray.instanceMethods

            Extend methods as instance methods

        @methods    - object of methods
    **/
    ModelArray.instanceMethods = function(methods){
        for (var name in methods){
            if (this.prototype[name] !== undefined){
                throw new Error('Falied to extend ModelArray.instanceMethod. ' +
                    this.prototype._name + '#' + name + '() already exists.');
            }
            this.prototype[name] = methods[name];
        }
    ...
    **/
    ...
    /**
         ModelArray.prototype.each
            Take the iterator and loop over all the rows

            See underscore for usage =>
                http://documentcloud.github.com/underscore/#each
            
        @iter
    **/
    ModelArray.prototype.each = function(iter) {
         _.each(this._rows, iter);
    }

    /**
         ModelArray.prototype.map
            Map this function to all the rows...
        ...
        @fn
    **/
    ModelArray.prototype.map = function(fn) {
         _.map(this._rows, fn);
    }

    /**
        ModelArray.prototype.filter
            Filter all the _rows by the iterator
            
            See underscore for usage =>
                http://documentcloud.github.com/underscore/#filter            
        
        @iter   - the iterator to filter on
    **/
    ModelArray.prototype.filter = function(iter) {
         return _.filter(this._rows, iter);
    }

    /**
         ModelArray.prototype.find
            Loops over all the _rows and returns the first one matching
            the truth test
        ...
        @iter
    **/
    ModelArray.prototype.find = function(iter) {
         return _.find(this._rows, iter);
    }
    
    /**
         ModelArray.prototype.first
         
            Returns the first n elements of the list. Defaults to 1.
            
            See underscore for usage =>
                http://documentcloud.github.com/underscore/#first
            
         
        @n - the number of rows to return; defaults to 1
    **/
    ModelArray.prototype.first = function(n) {
         return _.first(this._rows, n);
    }    

    /**
         ModelArray.prototype.last

            See underscore for usage =>
                http://documentcloud.github.com/underscore/#last

        @n - the number of rows to return; defaults to 1
    **/
    ModelArray.prototype.last = function(n) {
         return _.last(this._rows, n);
    }    
