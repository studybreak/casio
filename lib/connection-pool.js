...
var ConnectionPool = function (options) {
  ...
  // let's create the initial set of pool connections
  this.grow();
...
ConnectionPool.prototype.acquire = function () {
  ...
  // Create a new connection if we're getting close to the end of the queue.
  // Don't return it. It will be available after the connection is established.
  // Just use this acquire as a signal that the pool should potentially be grown.
  this.grow();
...
var headroom = 0.25;
ConnectionPool.prototype.grow = function () {
  // Only grow if we're getting close to empty.
  if (this.available.length > this.connections.length * headroom) return;

  var opts = this._options;
  var targetSize = Math.max(opts.minPoolSize, Math.min(opts.maxPoolSize,
      this.connections.length * (1 + headroom)));
  while (this.connections.length < targetSize) this.create();
};
...
ConnectionPool.prototype.create = function () {
  ...
  connection.connect(function (err) {
    ...
    if (err) {
      ...
      self.emit('log', 'verbose', 'Retrying first connection to Cassandra for host ' +
  ...
  if (connection) {
    ...
    self.emit('log', 'verbose', 'Growing Cassandra pool to ' +
        self.connections.length + ' for ' + self._options.host);
  }
